...
export default class FullEvent extends React.Component<props, state> {
    ...
    async getEvent(id) {
        const event = await fetch("https://active-directory.azurewebsites.us/loginEvents/singleEvent?id=" + id, {
            method: 'get',
            headers: new Headers({
    ...
    render() {
        return (
            <div className='col-md-10 col-md-offset-1' style={{ paddingBottom: '50px' }}>
                {this.state.event &&
                    <JSONInput
                        id='json'
                ...
                {!this.state.event &&
                    <Spinner notice="...loading event record..." />
                }
            </div>
        )
    }
